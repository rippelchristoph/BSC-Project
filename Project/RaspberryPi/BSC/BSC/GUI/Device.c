...
void DeviceDeviceClass_OnSetTemperature( DeviceDeviceClass _this, XInt32 value )
{
  ...
  EwTrace( "%s", EwConcatString( EwLoadString( &_Const0012 ), EwNewStringInt( _this->Temperature, 
    0, 10 )));
  {
    /*
       TO DO:
...
void DeviceDeviceClass_UpdateTemperature( DeviceDeviceClass _this, XInt32 aNewValue )
{
  if ( aNewValue != _this->Temperature )
  {
    ...
    EwTrace( "%s", EwConcatString( EwLoadString( &_Const0012 ), EwNewStringInt( 
      _this->Temperature, 0, 10 )));
    EwNotifyRefObservers( EwNewRef( _this, DeviceDeviceClass_OnGetTemperature, DeviceDeviceClass_OnSetTemperature 
      ), 0 );
  }
}
