...
*   _Next  - Pointer to the next object in the global chain of existing objects.
*     The global chain will be evaluated by the Garbage Collector to find and 
*     dispose garbage. Each time a new object instance has been created by the 
*     EwNewObject() function the new object will be inserted into this global 
*     chain.
*   _Cycle - _Cycle is a counter used internally by the Garbage Collector. 
*     The Garbage Collector stores there the current value of the global counter
*     'EwMarkCycle' if the affected object is reachable (alive). Then by 
*     comparing _Cycle with 'EwMarkCycle' the Garbage Collector can distinguish
*     between objects and garbage. Only objects they are previously marked 
*     (_Cycle == EwMarkCycle) are alive.
*
*******************************************************************************/
struct _vmt_XObject;
struct _obj_XObject
{
  ...
  struct _obj_XObject*         _Link;
  struct _obj_XObject*         _Next;
  int                          _Cycle;
};
...
*     for details).
*   _Mark - Pointer to the _Mark() method. The _Mark() method will be invoked
*     by the Garbage Collector to give the object a chance to find and mark all 
*     dependent objects as reachable. (see below for details).
*
*******************************************************************************/
...
*
* DESCRIPTION:
*   The type XObjectVariant defines the fundamental set of data fields used by
*   all object variants. An object variant serves as an extension of the origin
*   object - it does always belong to its origin object.
*
*   Each Embedded Wizard Chora class can be extended by derived class variants.
*   When during the runtime an object of such a class is created, the Embedded
*   Wizard RTE determines the appropriate variant of the class, creates an 
*   object variant of it and attaches the object variant to the origin object.
*   In this manner, the origin object can be extended by new behaviour without
*   any modifications of the origin class.
*
* ELEMENTS:
*   _VMT   - Pointer to the virtual method table (VMT) associated to the object
*     variant. The _VMT contains a list of methods supported by the variant and
*     an additional description of the class variant this objects belongs to.
*
*******************************************************************************/
struct _vmt_XObjectVariant;
struct _obj_XObjectVariant
{
  struct
  {
    const struct _vmt_XObjectVariant* VMT;
  } _;
};
typedef struct _obj_XObjectVariant* XObjectVariant;
...
*
* DESCRIPTION:
*   The type _vmt_XObjectVariant defines the fundamental structure of a virtual
*   method table VMT and will be used to describe a class variant. All Embedded
*   Wizard object variants use _vmt_XObjectVariant internally. This structure
*   can be compared to the VMT of the root class of all Embedded Wizard class
*   variants.
*
* ELEMENTS:
*   _Name  - The name of the class variant as a zero terminated string (for 
*     debugging purpose and test only).
*   _Ancestor - Reference to the next base class variant (the super class) of 
*     this class variant in the inheritance tree. If there is no further super
*     classes, _Ancestor will point to the global __vmt_XObjectVariant data 
*     structure. This global data structure is the root of all Embedded Wizard
*     object variants.
*   _SubVariants - Pointer to the null terminated list of sub-variants derived 
*     from this class variant. During the creation of an object, this list is 
*     evaluated in order to find and use the appropriate variant. 
*   _Size  - The size of an instance of this class variant in bytes. _Size will
*     be used by the EwAttachObjectVariant() function, when an object variant
*     is created.
*   _Init - Pointer to the _Init() method. The _Init() method will be invoked
*     to initialize a new object variant. (see below for details).
*   _ReInit - Pointer to the _ReInit() method. The _ReInit() method will be
*     invoked to re-initialize the object variant after the language has been
*     changed. (see below for details).
*   _Done - Pointer to the _Done() method. The _Done() method will be invoked
*     to deinitialize the object variant if its origin object is not used 
*     anymore. (see below for details).
*   _Mark - Pointer to the _Mark() method. The _Mark() method will be invoked
*     by the Garbage Collector to give the object variant a chance to find and
*     mark all dependent objects as reachable. (see below for details).
*
*******************************************************************************/
struct _vmt_XObjectVariant
{
  const char*                  _Name;
  const struct _vmt_XObject*   _Origin;
  const struct _XClassVariant* _SubVariants;
  int                          _Size;
  void                       (*_Init  )( XObject _this, XObject aLink, 
                                         XHandle aArg );
  void                       (*_ReInit)( XObject _this );
  void                       (*_Done  )( XObject _this );
  void                       (*_Mark  )( XObject _this );
};
extern const struct _vmt_XObjectVariant __vmt_XObjectVariant;
...
{
  int                               Style;
  const struct _vmt_XObjectVariant* Variant;
};
...
*   about the definition of the four fundamental methods _Init(), _Done(),
*   _ReInit() and _Mark() it is no necessary to define them explicitly.
*
...
#define EW_DEFINE_METHODS( aClass, aSuperClass )                               \
  ...
  void aClass##__Mark  ( aClass _this );                                       \
  struct _vmt_##aClass                                                         \
  {                                                                            \
    const char*                      _Name;                                    \
...
*
*******************************************************************************/
#define EW_DEFINE_CLASS( aClass, aSuperClass, aName )                          \
...
*     this class variant in the inheritance tree. If there is no further super 
*     classes, aSuperClass will get the name XObjectVariant (the root class of 
*     all object variants).
...
*   care about the definition of the four fundamental methods _Init(), _Done(),
*   _ReInit() and _Mark() it is no necessary to define them explicitly.
*
...
#define EW_DEFINE_VMETHODS( aClass, aSuperClass, aOriginClass )                \
  ...
  void aClass##__Mark  ( aOriginClass _this );                                 \
  struct _vmt_##aClass                                                         \
  {                                                                            \
    const char*                       _Name;                                   \
    const struct _vmt_##aOriginClass* _Origin;                                 \
...
*   aSuperClass - The name of the next base class variant (the super class) of 
*     this class in the inheritance tree. If there is no further super classes, 
*     aSuperClass will get the name XObjectVariant (the root class of all 
*     Embedded Wizard object classes).
*   aName       - The name of the class variant in the Chora notation with :: 
*     signs.
*
*******************************************************************************/
#define EW_DEFINE_VCLASS( aClass, aSuperClass, aOriginClass, aName )           \
  const struct _vmt_##aClass __vmt_##aClass =                                  \
  {                                                                            \
    aName,                                                                     \
    &__vmt_##aOriginClass,                                                     \
...
#define EW_CLASS_VARIANT( aStyle, aVariant )                                   \
  { aStyle, (const struct _vmt_XObjectVariant*)(const void*)&__vmt_##aVariant }

#define EW_STATIC_CLASS_VARIANT( aVariant )                                    \
  { 0, (const struct _vmt_XObjectVariant*)(const void*)&__vmt_##aVariant }
...
#define EW_DEFINE_DISPATCHER( aClass, aSuperClass )                            \
  ...
  {                                                                            \
    const char*                       _Name;                                   \
    const struct _vmt_##aClass*       _Origin;                                 \
    const XClassVariant*              _SubVariants;                            \
    int                               _Size;                                   \
    void (*_Init  )( aClass _this, XObject aLink, XHandle aArg );              \
    void (*_ReInit)( aClass _this );                                           \
...
*
* DESCRIPTION:
*   The method _Mark() will be invoked by the Garbage Collector to allow the 
*   object to mark itself and all dependend objects as reachable. Marked objects
*   will still keep alive so they will be not disposed by the Garbage Collector.
*
* ARGUMENTS:
*   _this - Pointer to the object to mark it as reachable.
*
* RETURN VALUE:
*   None
*
...
*
* DESCRIPTION:
*   The EwMarkObject() macro will be called from the EwReclaimMemory() 
*   function during the first phase of the garbage collection (see 
*   EwReclaimMemory()). The EwMarkObject() macro verifies wheter the object 
*   aObject has been already marked and if it's not it invokes the objects own
*   _Mark() method.
*
*   The invokation of the _Mark() method gets the object a chance to mark all
*   other objects which are in relationship to the object itself. In the case
*   of a relationship the _Mark() method of the affected object has to call the
*   EwMarkObject() method recursivelly and pass the pointer to the dependend 
*   object.
*
*   If the passed argument aObject is NULL, EwMarkObject() returns immediatelly.
*
* ARGUMENTS:
*   aObject - Pointer to an object to be marked.
*
* RETURN VALUE:
*   None
*
*******************************************************************************/
#define EwMarkObject( aObject )                                                \
  if ((aObject) && (((XObject)(aObject))->_Cycle != EwMarkCycle ))             \
  {                                                                            \
    ((XObject)(aObject))->_Cycle = EwMarkCycle;                                \
    ((XObject)(aObject))->_VMT->_Mark((XObject)(aObject));                     \
  }


/* Global garbage collector cycle counter */
extern int EwMarkCycle;
...
*
* DESCRIPTION:
*   The function EwMarkObjects() is usefull to mark all objects stored in an 
*   array as reachable. The function works similary to the EwMarkObject() 
*   function.
*
* ARGUMENTS:
*   aObjects - Pointer to the array with objects.
...
void EwMarkObjects
( 
  register XObject* aObjects, 
  int               aCount 
);

#define EwMarkObjects( aObjects )                                              \
  EwMarkObjects((XObject*)( aObjects ), sizeof( aObjects ) / sizeof( XObject ))
...
*   All reached objects will be marked (see EwMarkObject()). The traversing 
*   starts at the root set. The user can add and remove objects to or from the 
*   root set by calling the function EwLockObject() and EwUnlockObject(). As 
*   long as an object still keeps in the root set, the Garbage Collector can 
*   not dispose the objects and all other objects standing in the relationship 
*   to the object.
*
...
*
* DESCRIPTION:
*   The macro EwMarkRef() is necessary to mark an object which is stored in
*   the reference aRef.
*
* ARGUMENTS:
*   aRef  - Pointer to the reference containing the object.
*
* RETURN VALUE:
*   None
*
*******************************************************************************/
#define EwMarkRef( aRef )                                                      \
  if ((aRef).Object && ((XObject)((aRef).Object))->_Cycle != EwMarkCycle )     \
  {                                                                            \
    ((XObject)((aRef).Object))->_Cycle = EwMarkCycle;                          \
    ((XObject)((aRef).Object))->_VMT->_Mark((XObject)((aRef).Object));         \
  }
...
*
* DESCRIPTION:
*   The function EwMarkRefs() is usefull to mark all objects they are reachable
*   from the array of references. 
*
* ARGUMENTS:
*   aRefs  - Pointer to the array with references.
*   aCount - Number of entries within the array.
*
* RETURN VALUE:
*   None
...
#define EwMarkRefs( aRefs )                                                    \
  EwMarkRefs((XRef*)( aRefs ), sizeof( aRefs ) / sizeof( XRef ))
...
*
* DESCRIPTION:
*   The macro EwMarkSlot() is necessary to mark an object which is stored in
*   the slot aSlot.
*
* ARGUMENTS:
*   aSlot - Pointer to the slot containing the object.
*
* RETURN VALUE:
*   None
*
*******************************************************************************/
#define EwMarkSlot( aSlot )                                                    \
  if ((aSlot).Object && ((XObject)((aSlot).Object))->_Cycle != EwMarkCycle )   \
  {                                                                            \
    ((XObject)((aSlot).Object))->_Cycle = EwMarkCycle;                         \
    ((XObject)((aSlot).Object))->_VMT->_Mark((XObject)((aSlot).Object) );      \
  }
...
*
* DESCRIPTION:
*   The function EwMarkSlots() is usefull to mark all objects they are reachable
*   from the array of slots. 
*
* ARGUMENTS:
*   aSlots - Pointer to the array with slots.
*   aCount - Number of entries within the array.
*
* RETURN VALUE:
*   None
...
void EwMarkSlots
( 
  register XSlot*   aSlots, 
  int               aCount 
);

#define EwMarkSlots( aSlots )                                                  \
  EwMarkSlots((XSlot*)( aSlots ), sizeof( aSlots ) / sizeof( XSlot ))
...
*     output.
*   aPrecision - The aPrecision argument specifies the number of digits after 
*     the decimal point. 
*
* RETURN VALUE:
...
*   function works similary to the ANSI 'C' printf() function. This function
*   should be implemented by the programer in order to output trace messages on
*   the debug console, etc. 
*
* ARGUMENTS:
*   aFormat - Zero-terminated string containing the message and the printf() 
*     escape sequences.
*
* RETURN VALUE:
*   None
...
*
* DESCRIPTION:
*   The function EwFormat() provides a plattform independent version of the 'C'
*   standard library function sprintf( aDstPtr, aFormat, ... ).
*
*   This function formates a string depending on the aFormat parameters as it
*   does sprintf() function.
*
* ARGUMENTS:
*   aDstPtr - Pointer to a buffer where the resulting formated string is stored.
*   aSize   - Size of the buffer in bytes.
*   aFormat - Zero-terminated string containing the printf() escape sequences.
*
* RETURN VALUE:
*   None
*
